...
- **Pushing IVR-collected data into the AI session** (so the AI agent knows what the flow already learned — verified customer ID, menu choice, language): a small flow-invoked Lambda that (1) reads `ContactId` from the flow event, (2) `connect:DescribeContact` → `Contact.WisdomInfo.SessionArn`, (3) `qconnect UpdateSessionData(assistantId, sessionId=sessionArn, data=[{key,value},…])` with the flow's `Details.Parameters`. Invoke it via the Lambda block *after* the Q in Connect block and *before* handing the participant to the bot. This Lambda is generic/business-agnostic — write it once, drive it entirely from flow parameters.
- ⚠ **IAM actions for Q in Connect use the `wisdom:` prefix** — `qconnect:*` does **not** exist as an IAM action prefix; a policy with `qconnect:UpdateSessionData` yields AccessDenied at runtime even though the CLI/SDK service is called `qconnect`. E.g. the Lambda above needs `wisdom:UpdateSessionData` (+ `connect:DescribeContact`).
- **AI prompt authoring gotchas:** each `{{$.xxx}}` system variable may appear **only once** per prompt (duplicates are rejected); for voice, keep prompt-instructed output TTS-friendly (no markdown or special characters; instruct how numbers/dates are read aloud).
...
- **RTC detection in flows:** tool name + inputs land in **Lex session attributes**. Check `Namespace=Lex, Key=Session attributes, Session Attribute Key=Tool` (values `Complete`/`Escalate`/custom); copy inputs (e.g. `$.Lex.SessionAttributes.escalationSummary`) to contact attributes for routing/screenpop; optionally surface to the agent via **Set event flow** → Default flow for agent UI. ⚠ The values are a **verbatim, case-sensitive contract** between the AI prompt and the flow's `Compare` operands — `Complete` and `Escalate` exactly, never invented synonyms (`END_CALL`, `DONE`, `ESCALATE`, `TRANSFER`); any custom extension (`EscalateBilling`) must be defined identically on both sides. See `contact-flows.md` §14.5.
